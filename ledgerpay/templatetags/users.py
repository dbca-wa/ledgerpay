...
from ledgerpay import helpers as ledgerpay_helpers
...
def is_ledgerpay_admin(context):
    # checks if user is an AdminUser
    request = context['request']
    return ledgerpay_helpers.is_ledgerpay_admin(request)

@register.simple_tag(takes_context=True)
def is_internal(context):
    # checks if user is a departmentuser and logged in via single sign-on
    request = context['request']
    return ledgerpay_helpers.is_internal(request)
...
def is_model_backend(context):
    ...
    request = context['request']
    return ledgerpay_helpers.is_model_backend(request)
